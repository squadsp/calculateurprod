...
import { DEFAULT_CADRE_ALU_SETTINGS, loadCadreAluSettings, saveCadreAluSettings, resetCadreAluSettings, type CadreAluSettings } from "@/lib/cadresAluSettings";
...
function CadresAluSettingsEditor() {
  const [s, setS] = useState<CadreAluSettings>(DEFAULT_CADRE_ALU_SETTINGS);
  const [msg, setMsg] = useState<string | null>(null);

  useEffect(() => { setS(loadCadreAluSettings()); }, []);
  ...
  const setCol = (key: string, patch: Partial<CadreAluSettings["columns"][number]>) =>
    setS((prev) => ({
      ...prev,
